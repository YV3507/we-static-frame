...
- 只有**松散场景目录**（`scene.json` 可直接读）才能用；`scene.pkg` 请先点「解包」
  —— 服务端会**明确报错**而不是静默忽略勾选。
- 用 `allow/deny` 或逐效果后端时不受此限（走的是策略层）。

### 解包（pkg → 松散目录）

`POST /api/unpack {input}` 把 `scene.pkg` 落成目录（默认 `webui/tmp/unpacked/<id>/`）。
...
- 逐对象/逐效果勾选需要**松散场景目录**：界面上的「解包」按钮会把 pkg 落成目录（见上）。
