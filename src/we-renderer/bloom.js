...
// 2. 合成: 原图 + bloom 4 角平均×0.25 → 线性化 lin() → ×曝光 → linear→sRGB 显示编码
//    (官方 combine_hdr.frag:32/43 把 lin(albedo) 写进 HDR/线性渲染目标, 显示阶段才编码;
//     本渲染器的 canvas 就是交付的 8bit sRGB 帧, 故必须在此显式回编码 — 见 .test-cache/fix-bloom-srgb.md)
...
export function applyBloom(canvas, gen, getNum) {
  ...
  const exposure = 1;
  // linear→sRGB 显示编码 —— 这是官方 combine_hdr.frag 之后唯一缺失的环节:
  // 官方 HDR 分支的 lin(...) 写进 **线性/HDR 渲染目标**, 由引擎显示阶段编码;
  // 而 canvas.data 在这里已是最终交付的 8bit sRGB 画布 (scene-render-worker.mjs
  // 直接 encodePng), 少这一步 ⇒ 线性值被当成显示值 ⇒ 整帧被压暗 (~4.4×)。
  ...
  for (let y = 0; y < H; y++) {
    ...
    for (let x = 0; x < W; x++) {
      ...
      if (isHdr) {
        // HDR: lin(albedo) + bloom → ×曝光 → linear→sRGB 编码 (引擎 combine_hdr.frag
        // 的线性输出在显示阶段编码; 这里手动补上, 否则整帧偏暗)
        const r = lin(data[i] / 255) + br;
        const g2 = lin(data[i + 1] / 255) + bg;
        const b2 = lin(data[i + 2] / 255) + bb;
        data[i] = Math.min(255, Math.round(toSrgb(sat(r) * exposure) * 255));
        data[i + 1] = Math.min(255, Math.round(toSrgb(sat(g2) * exposure) * 255));
        data[i + 2] = Math.min(255, Math.round(toSrgb(sat(b2) * exposure) * 255));
      } else {
...
export function installBloom(proto) {
  Object.assign(proto, {
    _applyBloom(gen) {
        ...
        const exposure = 1;
        // linear→sRGB 显示编码 (与官方 combine_hdr.frag 的显示阶段对应; 见本文件顶部说明)
        const toSrgb = (v) => v <= 0.0031308 ? v * 12.92 : 1.055 * Math.pow(v, 1 / 2.4) - 0.055;
        ...
        for (let y = 0; y < H; y++) {
          ...
          for (let x = 0; x < W; x++) {
            ...
            const bb = (blur[o + 2] + blurHdr[o + 2]) * eff;
            if (isHdr) {
              // HDR: lin(albedo) + bloom → 曝光 → linear→sRGB 编码 (引擎 combine_hdr;
              // 官方把线性值写进 HDR/线性渲染目标由显示阶段编码, 本渲染器画布即最终
              // 8bit sRGB 帧, 必须在此补齐编码, 否则整帧被压暗)
              const r = lin(data[i] / 255) + br;
              const g2 = lin(data[i + 1] / 255) + bg;
              const b2 = lin(data[i + 2] / 255) + bb;
              data[i] = Math.min(255, Math.round(toSrgb(sat(r) * exposure) * 255));
