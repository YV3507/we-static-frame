...
// JPEG 无 alpha 通道: fallback 必须是不透明黑 (RGB 已为 0, alpha 必须填 255),
// 零缓冲的 alpha=0 会被 blitter 按透明跳过, 背景对象消失后漏出 clearcolor
// (花壁纸 2934788040 实证: 角点由 179,179,179 变黑).
function opaqueBlackRgba(width, height) {
...
export function decodeJpeg(bytes) {
  ...
  try {
    ...
    if (e && e.message && e.message.startsWith('jpeg:') && e._dims) {
      ...
      return { width, height, rgba: opaqueBlackRgba(width, height) };
    }
    throw e;
  }
}
