...
export const fx = {
    ...
    effectLightshafts(tex, c, t, pass, ef, name) {
        ...
        // 取值改为**从壁纸自带的 shader 源码**读 (effects/lightshafts.frag 的
        // // [COMBO] {"combo":"RAYMODE","default":0} / {"combo":"RENDERING","default":0}),
        // 材质/scene 的显式覆盖优先 —— 旧实现只看 scene 内联 combos, 材质声明会漏。
        // 非默认值 → 记一次 degraded (对象保留): 光柱几何与 BLENDMODE 混色仍成立,
        // 跳过会整层失去光照, 近似与官方的差值只在 falloff 曲线/着色模式。
        {
          // 注: BLENDMODE 不列入 —— 本内核按 combos.BLENDMODE 动态执行 (非固定变体)
          const impl = { RAYMODE: 0, RENDERING: 0, RAYCORNER: 0, WRITEALPHA: 0 };
          const aud = this._fxJsonComboAudit ? this._fxJsonComboAudit(ef, name, impl)
            : { found: false, bad: [], values: {}, reason: 'effect.json 读取器不可用' };
          if (!aud.found) {
            degradedOnce(this, 'effect:lightshafts:combo',
              'lightshafts 无法从壁纸读取 shader combo 声明（' + (aud.reason || 'effect.json 无匹配 shader') + '），'
              + '按 RAYMODE=0/RENDERING=0 近似渲染（对象保留）');
          } else if (aud.bad.length) {
            degradedOnce(this, 'effect:lightshafts:combo', 'lightshafts 未支持 combo '
