...
export function scratchGet(kind, len) {
  ...
  if (buf) {
    ...
    if (_scopes) for (const s of _scopes) s.push(buf); // 修复: 借出登记 (抛错路径归还用)
    return buf;
  }
...
// ── 借出作用域 (修复: 效果内核抛错时的泄漏) ─────────────────────────
// 效果内核在 scratchGet 之后、scratchPut 之前抛错 (采样越界/解析异常等) 时,
// 它借出的整帧缓冲会一直停在 out 状态。effects.js 包住每次效果调用:
//   begin → 调用内核; 抛错 → release 统一归还; 无论成败 → end 出栈。
// 成功路径不归还 (内核自身已归还中间缓冲), 链输出必须保持 out 由下一帧召回。
let _scopes = null; // 活动借出作用域栈 (渲染期间深度 0/1)

// 开启一个借出作用域 (范围 = 一次效果内核调用)
export function scratchScopeBegin() {
  if (!_scopes) _scopes = [];
  const scope = [];
  _scopes.push(scope);
  return scope;
...
// 归还本次作用域借出且仍占用的缓冲 (幂等: 成功路径已归还的会被忽略)
export function scratchScopeRelease(scope) {
  if (scope) for (const b of scope) scratchPut(b);
}

// 出栈 (不归还) — 成功路径由内核自行归还, 链输出保持 out
export function scratchScopeEnd(scope) {
  if (!_scopes) return;
  const i = _scopes.lastIndexOf(scope);
  if (i >= 0) _scopes.splice(i, 1);
}

// 帧首召回: 把所有仍在"借出"状态的缓冲标回可用 (上一帧的效果链输出此时已
// 无引用)。每帧 render() 开头调用一次 (core.js render), 让池跨帧复用 —
// 修复: 此前 scratchRecallAll 零调用点 ⇒ 每效果对象每帧滞留一块整帧缓冲。
