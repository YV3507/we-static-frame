 ...
 */
// ── varying 类型对齐 ─────────────────────────────────────────────────────────
// WE 的 frag/vert 对同名 varying 的类型**不保证一致** —— 实测 color_grading:
//   frag: varying vec2 v_TexCoord;    vert: varying vec4 v_TexCoord; (只写 .xy)
// CPU 解释器不检查 (自己插值 varying 并按分量取用), 但 WebGL 会直接拒绝链接:
//   "Types of varying 'v_TexCoord' differ between VERTEX and FRAGMENT shaders."
// 这里以 **fragment 的声明为准** 改写 vertex 侧声明 —— frag 用不到的分量对语义
// 没有影响。仅当 vert **未引用**将被丢掉的分量时才降级; 有引用就保持原样,
// 让链接失败自然回退 CPU (宁可慢, 不可错)。
function parseVaryingDecls(src) {
  const out = new Map();
  const re = /\bvarying\s+(vec[234]|float|mat[234])\s+(\w+)\s*;/g;
  let m;
  while ((m = re.exec(src))) out.set(m[2], m[1]);
  return out;
}

function reconcileVaryings(fSrc, vSrc) {
  const fv = parseVaryingDecls(fSrc);
  const vv = parseVaryingDecls(vSrc);
  let out = vSrc;
  for (const [name, ftype] of fv) {
    const vtype = vv.get(name);
    if (!vtype || vtype === ftype) continue;
    // 目前只处理可安全降级的组合: vert vec4 → frag vec2 (丢掉的 zw 无人引用)
    if (vtype === 'vec4' && ftype === 'vec2') {
      if (new RegExp('\\b' + name + '\\s*\\.\\s*[zw]', 'i').test(vSrc)) continue;
      out = out.replace(new RegExp('(\\bvarying\\s+)vec4(\\s+' + name + '\\s*;)'), '$1vec2$2');
    }
  }
  return out;
}
...
export function buildProgram(gl, fragPre, vertPre) {
  ...
  let vSrc = vertPre ? toWebGLSource(vertPre, 'vertex').source : defaultVertFor(fragPre);
  vSrc = reconcileVaryings(fSrc, vSrc);
...
function defaultVertFor(fragPre) {
  ...
  for (const v of varyings) {
    if (v.name === 'v_TexCoord') {
      // 按 frag 声明的类型赋值 —— 声明 vec2 却赋 vec4 会编译失败 (此前对声明
      // vec2 v_TexCoord 的 frag 一律赋 vec4, 同类 shader 全部编译不过)
      parts.push(v.type === 'vec4'
        ? '  v_TexCoord = vec4(a_pos * 0.5 + 0.5, a_pos * 0.5 + 0.5);'
...
export function runEffectOnGL({ fragPre, vertPre = null, u = {}, width, height }) {
  ...
  let nextUnit = 0;
  // wrap 语义必须与 CPU 解释器一致 (integration.js): 主图 g_Texture0 = CLAMP,
  // 辅助槽 (mask/位移/噪声) = REPEAT —— uploadTextureData 统一设 CLAMP, 会让
  // 采样跨 [0,1] 的位移/遮罩类效果与 CPU 产生可见差异 (实测 2% 像素 >32/255)。
  // 这里在每次绑定后按槽位重设 wrap (texParameteri 只是状态切换, 不重传纹理)。
  const wrapFor = (name, texData) => {
    if (name !== 'g_Texture0') return gl.REPEAT;
    return texData && texData.__glslRepeat ? gl.REPEAT : gl.CLAMP_TO_EDGE;
  };
  ...
  for (const [name, texData] of texEntries) {
    ...
    // 按槽位重设 wrap (同纹理被多槽位共用时以最后一次为准 —— 与 CPU 的
    // img.__glslRepeat 标记语义一致: img 同时当主图与辅助槽时两者都用 REPEAT)
    const wrap = wrapFor(name, texData);
    gl.activeTexture(gl.TEXTURE0 + unit);
    gl.bindTexture(gl.TEXTURE_2D, texCacheGet(texData) || gl.getParameter(gl.TEXTURE_BINDING_2D));
