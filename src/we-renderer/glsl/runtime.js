...
// ── inout/out **向量**参数的就地写 (官方 shader 的 out vec3 形参语义) ──────────
// GLSL 的 `p op= x` / `p = x` (p 为 out/inout vec) 必须写回**调用方**的同一数组;
// 而 vec 在 JS 里是数组值 ⇒ `p = <新数组>` 只重绑形参, 调用方看不到。实测
// common_fragment.h:61 ComputeLightSpecular 的 `inout vec3 specularResult` 因
// `specularResult += …` 整条丢失 ⇒ generic.frag 的镜面项恒为 0 (材质变暗)。
// 下列辅助按 GLSL 语义**就地**写回, 数值序列与 _vaddEq 等逐位一致。
export function _vInto(a, b, op) {
  const n = a.length;
  if (typeof b === 'number') {
    for (let i = 0; i < n; i++) a[i] = op === 'add' ? a[i] + b : op === 'sub' ? a[i] - b : op === 'mul' ? a[i] * b : a[i] / b;
    return a;
  }
  for (let i = 0; i < n; i++) {
    const bv = i < b.length ? b[i] : 0;
    a[i] = op === 'add' ? a[i] + bv : op === 'sub' ? a[i] - bv : op === 'mul' ? a[i] * bv : a[i] / bv;
  }
  return a;
}
export function _vAssign(a, b) {
  if (typeof b === 'number') { a.fill(b); return a; }
...
// HLSL 拼写别名 — 官方壁纸自带 material shader 用 `frac` / `lerp`
// (实测语料: `frac` 13 处, 见 .test-cache/fix-glsl-materials.md §1)
...
export function runtimeObject(texSample) {
  const rt = {
    ...
    texSample2DLod: makeTexSample(texSample), // HLSL 拼写别名 (官方 genericimage2.frag:132 等)
  };
  return rt;
}
