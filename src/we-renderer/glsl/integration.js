...
export function installGlsl(proto) {
  ...
  proto._readGlslShader = function (name, ef) {
    ...
    // pkg 内候选路径 (按优先级):
    //   ① workshop 效果: ef.file = effects/workshop/<id>/<name>/effect.json
    //      → shaders/workshop/<id>/effects/<name>.(frag|vert)   ← 实测 Angel Mail 的
    //      texture_override / geometric_transform / auto_sway 等均为此形态; 旧实现只试
    //      ② ⇒ 这些效果全部"shader 未找到"被跳过 (组件退化为纯色/白块)。
    //   ② 官方风格: shaders/effects/<name>.(frag|vert)
    const stems = [];
    const wid = /workshop\/(\d+)\//.exec((ef && ef.file) || '');
    if (wid) stems.push('shaders/workshop/' + wid[1] + '/effects/' + name);
    stems.push('shaders/effects/' + name);
    const stem = stems[0];
    let frag = '';
    for (const s of stems) { try { frag = this.pkg.readText(s + '.frag') || ''; } catch {} if (frag) break; }
    ...
    let vert = '';
    for (const s of stems) { try { vert = this.pkg.readText(s + '.vert') || ''; } catch {} if (vert) break; }
    ...
    if (!frag) this.log('GLSL shader 未找到: ' + stems.join(' | ') + '.frag (' + name + ')'); // P1-45: 失败记日志 (源码缓存防逐帧重复)
    const rec = { frag, vert };
  ...
  proto._renderGlslEffect = function (compiled, img, ef, w, h, t, name) {
    try {
      ...
      if (out.pixelErrors) {
        // 报错即回退 (用户裁决: 宁可"该效果不应用", 也不要部分像素是垃圾的输出;
        // 少量算力浪费用作试错成本): 逐像素异常说明该效果在本 shader/参数下不可靠,
        // 整体丢弃、保留输入图 —— 与 effects.js 的退化保护同一策略。
        this.log('GLSL 效果 ' + name + ': ' + out.pixelErrors + ' 像素异常 → 整体回退不应用 (' + (out.lastError || '') + ')'); // F-12
        this._degraded(null, 'effect:' + name, 'GLSL 逐像素异常 ' + out.pixelErrors + ' 处，已整体回退不应用: ' + (out.lastError || ''));
        return img;
      }
      return out;
    } catch (e) {
