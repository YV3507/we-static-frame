...
const BUILTIN = new Set([
  ...
  // HLSL 名称别名 — 官方壁纸自带 shader 用的是 HLSL 拼写 (实测 material shader 语料:
...
class Transpiler {
  constructor(ast, stage) {
    ...
    this.attributes = []; // 顶点属性声明序 (数据驱动材质解释器按它建 attribute 缓冲)
    ...
    for (const node of this.ast.program || []) {
      if (node.type === 'declaration_statement') {
        ...
        for (const d of dl.declarations || []) {
          ...
          // 顶点属性清单 (只在 #if 求值后的 AST 上取 ⇒ SKINNING 等未启用分支不出现)
          if (kind === 'attribute') this.attributes.push({ type, name: d.identifier.identifier, array: isArr });
        }
  ...
  generate() {
    ...
    if (this.stage === 'vertex') {
      ...
      // 数据驱动材质解释器需要**读回** vert 程序输出的 gl_Position (官方 .vert 的
      // 唯一顶点位置出口 —— 此前 executor 只取 varying, 顶点位置被丢弃)。
      // 用 __out 盒持有: `gl_Position` 在生成代码里一律解析为 `__out.gl_Position`
      // (见 expr/lvalue), 因此 `gl_Position = ...` 的写入对调用方可读。
      lines.push('var __out = { gl_Position: null };');
    }
    for (const node of this.ast.program || []) {
    ...
    if (this.stage === 'fragment') {
      ...
      lines.push(`return { main: main, __out: __out, attributes: ${JSON.stringify(this.attributes)}, __initGlobals: ${initRef} };`);
    }
    return lines.join('\n');
  }
  ...
  functionCode(node) {
    ...
    const copies = params
      ...
      .join('\n');
    // out/inout **向量/矩阵** 参数: 整个变量赋值 / 复合赋值必须**就地写回**调用方数组
    // (否则 `p = <新数组>` 只重绑形参 ⇒ 调用方看不到; 见 runtime.js:_vInto 注释)
    const inoutRefs = new Set(params
      .filter((p) => {
        const t = this.typeOf(p.specifier);
        return (VEC_SIZE[t] || MAT_SIZE[t]) && (p.qualifier || []).some((q) => q.token === 'inout' || q.token === 'out');
      })
  ...
  assignStmt(node, ctx) {
    ...
    if (op === '=') {
      const rhs = this.expr(node.right, ctx);
      // out/inout 向量形参: 就地拷贝 (重绑形参调用方看不到)
      if (left.type === 'identifier' && ctx.inoutRefs && ctx.inoutRefs.has(left.identifier)) {
        return `__rt._vAssign(${lv}, ${rhs.code});`;
      }
      return `${lv} = ${rhs.code};`;
    }
    ...
    if (VEC_SIZE[lt] || MAT_SIZE[lt]) {
      // out/inout 向量形参: 就地复合写 (同上)
      if (left.type === 'identifier' && ctx.inoutRefs && ctx.inoutRefs.has(left.identifier)) {
        const kind = { '*=': 'mul', '+=': 'add', '-=': 'sub', '/=': 'div' }[op];
        if (kind) return `__rt._vInto(${lv}, ${rhs.code}, '${kind}');`;
      }
  ...
  expr(node, ctx) {
    ...
    switch (node.type) {
      case 'identifier': {
        const nm = node.identifier;
        let code;
        // 顶点阶段 gl_Position → __out 盒 (数据驱动材质解释器读回顶点位置)
